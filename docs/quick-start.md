...
> <span v-pre><span class="doc-pro-pill">Pro</span> Pro lets you embed dynamic placeholders like <code>{{date}}</code>, <code>{{user}}</code>, <code>{{ip}}</code>, <code>{{copyright}}</code> — see <a href="/docs/third-party-integrations#dynamic-placeholders">Pro features → Dynamic placeholders</a>.</span>
